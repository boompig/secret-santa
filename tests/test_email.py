...
from secret_santa.email_utils import (
    create_emails,
    get_email_fname,
    get_email_text,
    send_all_emails,
...
def test_get_email_text():
    ...
    with tempfile.TemporaryDirectory() as output_dir:
        for giver, receiver in pairings.items():
            format_dict = {"giver_name": giver, "receiver_name": receiver}
...
def test_create_emails_unencrypted():
    ...
    with tempfile.TemporaryDirectory() as output_dir:
        create_emails(
            pairings, email_template_fname=EMAIL_TEMPLATE_FNAME, output_dir=output_dir
        )
    ...
    with tempfile.TemporaryDirectory() as output_dir:
        ...
        for name, email in NAMES.items():
            mailer.send_email.assert_any_call(
                email_subject, email_contents[name], email
            )
